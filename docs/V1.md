...
│   │   ├── ConversationList.ts    # Conversation sidebar/list
│   │   └── components/            # Reusable UI components
│   ├── agent/
│   │   ├── AgentGraph.ts          # LangGraph agent definition
│   │   ├── AgentState.ts          # State schema for agent
│   │   ├── AgentTools.ts          # Vault access tools
│   │   └── AgentPrompts.ts        # System prompts and identity
│   ├── services/
│   │   ├── VaultService.ts        # NEW: Vault read operations
│   │   ├── ConversationService.ts # Thread/conversation management
│   │   └── CheckpointService.ts   # NEW: LangGraph checkpoint storage
│   ├── models/
│   │   ├── Conversation.ts        # Conversation/thread model
│   │   └── Message.ts             # Message model
│   ├── utils/
│   │   ├── markdown.ts            # Markdown rendering utilities
│   │   ├── export.ts              # Export functionality
│   │   ├── vaultSearch.ts         # NEW: Vault search utilities
│   │   └── linkParser.ts          # NEW: Parse Obsidian links
│   ├── types.ts                   # TypeScript interfaces
│   └── settings.ts                # Settings tab
├── styles.css                     # Plugin styles
├── manifest.json                  # Plugin metadata
├── package.json                   # NEW: Includes LangGraph dependencies
└── ...
```
...
├── checkpoints/
│   ├── {thread-id-1}/          # Thread-specific checkpoints
│   │   ├── checkpoint-1.json
│   │   └── checkpoint-2.json
│   └── {thread-id-2}/
...
│ 📌 Pinned Project           │
│   "Let me help with..."     │
│   5 messages • 230 tokens   │
│   Today at 2:30 PM          │
├─────────────────────────────┤
│ ● Research Ideas            │
│   "Here are some..."        │
│   12 messages • 890 tokens  │
│   Yesterday                 │
├─────────────────────────────┤
│ Code Review Discussion      │
│   "The function looks..."   │
│   8 messages • 450 tokens   │
│   Oct 28, 2024              │
└─────────────────────────────┘
```
...
→ Generate UUID, initialize empty messages[]
→ StorageService.saveConversation()
→ Update UI, switch to new conversation
```
...
→ Update token counts
→ StorageService.saveConversation() (debounced)
→ Update UI
```
...
→ Load conversation from storage/cache
→ Update ChatView with messages
...
```
User clicks "Export"
→ ConversationService.exportConversation()
→ Format as markdown
→ Create note in vault (or copy to clipboard)
...
- ✓ Agent identifies as "Obsidian Agent"
- ✓ Agent can search vault by filename, content, and tags
- ✓ Agent can read files from vault
- ✓ Agent can list vault structure
- ✓ Conversations persist across sessions using LangGraph checkpoints
- ✓ Users can create and manage multiple conversation threads
- ✓ Users can rename and delete threads
- ✓ Thread history maintained correctly
- ✓ Code blocks have syntax highlighting
- ✓ Conversations can be exported to vault

### Non-Functional Requirements
- ✓ Plugin loads in < 3 seconds
- ✓ Vault search completes in < 2 seconds for vaults up to 1000 notes
- ✓ Thread switching is instant (<200ms)
- ✓ No UI freezing during vault operations
- ✓ Data integrity maintained across crashes
- ✓ Works efficiently with conversations up to 100 messages

### Agent Behavior
- ✓ Agent proactively uses vault tools when appropriate
- ✓ Agent accurately quotes from vault files
- ✓ Agent explains read-only limitations when asked to modify
- ✓ Agent maintains context across conversation
- ✓ Agent provides helpful responses about vault structure

### User Experience
- ✓ Clear indication when agent is accessing vault
- ✓ Intuitive thread management
- ✓ Helpful error messages for vault operations
- ✓ Smooth agent responses
...
### Block 1: LangGraph Foundation (Week 1)
**Goal**: Get a basic LangGraph agent working with in-memory persistence

**Tasks**:
1. Install dependencies
   - Add `@langchain/langgraph`, `@langchain/anthropic`, `@langchain/core`, `zod`
   - Update `package.json` and verify builds
2. Create agent state schema
   - Define `AgentState` with messages array
   - Add `vaultContext` tracking
3. Build minimal agent
   - Create simple agent graph (agent node → tool node → agent)
   - Use `MemorySaver` for in-memory checkpointing
   - Create one test tool (e.g., "echo")
4. Test persistence
   - Verify agent maintains context across multiple invocations
   - Test with different thread IDs
5. Integrate with ChatView
   - Replace existing ChatService with LangGraph agent
...
- [ ] Can switch between conversations (thread IDs)
...
Block 1 (LangGraph Foundation)
    ↓
Block 2 (Vault Service) ← Can start in parallel with Block 1 after day 2
    ↓
Block 3 (Vault Tools)
    ↓
...
## Critical Path
**Blocks 1 → 3 → 4** are on the critical path. Block 2 can be developed in parallel with Block 1 after the basic agent structure is established. Blocks 5 and 6 build on the foundation but can overlap (e.g., start polish work while finishing UI).
